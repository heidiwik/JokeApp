...
namespace APIHelpers
{
    ...
    {
        const string url = "https://v2.jokeapi.dev/joke/";

        public static async Task<Joke> GetRandomJoke(string category)
        {
            string fullUrl = url + "/" + category;
            Joke response = await ApiHelper.RunAsync<Joke>(fullUrl, "");
            return response;
        }
    }
